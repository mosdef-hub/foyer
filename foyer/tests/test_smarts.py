...
def _rule_match(top, typemap, smart, result):
    rule = SMARTSGraph(
        name="test", parser=PARSER, smarts_string=smart, typemap=typemap
    )
...
def test_ring_count():
    ...
    rule = SMARTSGraph(
        name="test", parser=PARSER, smarts_string="[#6;R2]", typemap=typemap
    )
    ...
    rule = SMARTSGraph(
        name="test", parser=PARSER, smarts_string="[#6;R1]", typemap=typemap
    )
    match_indices = list(rule.find_matches(fused_graph, typemap))
